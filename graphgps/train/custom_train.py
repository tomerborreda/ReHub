...
from graphgps.train import metric_utils
...
def eval_epoch(logger, loader, model, split='val'):
    ...
    if cfg.rehub.logging.plot_metrics:
        utilization = [[] for _ in range(cfg.gt.layers)]
        bhattacharyya = [[] for _ in range(cfg.gt.layers)]
    ...
    for batch in loader:
        ...
        if cfg.rehub.logging.log_cuda_time_and_gpu_memory:
        ...
        if cfg.rehub.logging.log_cuda_time_and_gpu_memory:
            ...
            time_cuda = start.elapsed_time(end)
            extra_stats['time_cuda_ms'] = time_cuda
            extra_stats['max_memory_gb'] = float(max_memory) / (1024 ** 3)
        ...
        if cfg.rehub.logging.plot_metrics:
            for i in range(len(utilization)):
                utilization[i].extend(batch.log_utilization[i].cpu().tolist())
                bhattacharyya[i].extend(batch.log_bhattacharyya[i].cpu().tolist())

    if cfg.rehub.logging.plot_metrics:
        if split == 'val':
            metric_utils.generate_utilization_graph(utilization[-1], len(loader.dataset), split)
            metric_utils.generate_bhattacharyya_graph(bhattacharyya[-1], len(loader.dataset), split)
            metric_utils.generate_utilization_cumulative_graph(utilization, len(loader.dataset), split)
            metric_utils.generate_bhattacharyya_cumulative_graph(bhattacharyya, len(loader.dataset), split)
