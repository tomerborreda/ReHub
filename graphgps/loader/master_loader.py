...
from torch_geometric.datasets import (Actor, Coauthor, GNNBenchmarkDataset, Planetoid,
...
from graphgps.loader.dataset.large_random_graph import LargeRandomRegularGraphsDataset
...
from graphgps.transform.transforms import (pre_transform_in_memory,
                                           ...
                                           clip_graphs_to_size)
from graphgps.transform.spoke_hub_clusters import spoke_hub_clusters
...
def load_dataset_master(format, name, dataset_dir):
    ...
    elif format.startswith('Custom-'):
        custom_dataset_id = format.split('-', 1)[1]
        dataset_dir = osp.join(dataset_dir, custom_dataset_id)
        if custom_dataset_id == 'LargeRandomRegularGraphs':
                dataset = preformat_LargeRandomRegularGraphs(dataset_dir, name)

        else:
            raise ValueError(f"Unexpected Custom Dataset identifier: {format}")
    ...
    if cfg.rehub.prep:
        start = time.perf_counter()
        logging.info(f"Pairing Spokes with Hubs ...")
        pre_transform_in_memory(dataset,
                                partial(spoke_hub_clusters,
                                        num_hubs=cfg.rehub.num_hubs,
                                        num_hubs_type=cfg.rehub.num_hubs_type),
                                show_progress=True
                                )
        elapsed = time.perf_counter() - start
        timestr = time.strftime('%H:%M:%S', time.gmtime(elapsed)) \
...
def preformat_LargeRandomRegularGraphs(dataset_dir, name):
    """Load and preformat LargeRandomRegularGraphs dataset.
    ...
    Returns:
        PyG dataset object
    """
    if name.startswith('LargeRandomGraph-'):
        graph_size = int(name.split('-')[1])
        dataset = LargeRandomRegularGraphsDataset(root=dataset_dir, num_nodes_in_graph=graph_size, name=name)
    else:
        raise ValueError(f"Loading dataset '{name}' is not supported.")
    s_dict = dataset.get_idx_split()
    dataset.split_idxs = [s_dict[s] for s in ['train', 'val', 'test']]
    return dataset
