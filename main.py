...
from graphgps.cmd_args import parse_args
...
def wandb_args_process(wandb_json):
    """Process the wandb_json argument"""

    with open(os.path.join(os.path.dirname(os.path.abspath(__file__)), wandb_json), 'r') as json_file:
    ...
    for key, value in json_dict.items():
        if isinstance(value, dict):
            for inner_key, inner_value in value.items():
                processed_args.extend([f"{key}.{inner_key}", inner_value])
        else:
...
if __name__ == '__main__':
    ...
    args = parse_args()
    if args.wandb_json:
        args.opts.extend(wandb_args_process(args.wandb_json))
    # Load config file
    set_cfg(cfg)
    load_cfg(cfg, args)
